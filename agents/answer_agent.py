    ...
    def generate_answer(
        ...
        if has_code:
            rules = (
                ...
                "2. Форматируй каждый файл в виде списка: название файла жирным (**file.py**:), со следующей строки пиши его описание, а еще ниже с новой строки указывай 'Источник: [slug]'.\n"
                "3. СТРОГО используй оригинальные названия файлов из контекста для источника (например, [code-catalog-py]). Ставь 1-2 наиболее точные ссылки [slug] на предложение. ЗАПРЕЩЕНО ставить много ссылок подряд!\n"
    ...
    async def agenerate_answer(
        ...
        if has_code:
            rules = (
                "1. ФОРМАТ ДЛЯ КОДА: Выведи ответ с красивым форматированием. Обязательно добавь заголовок (например, ### Питон-файлы по продукту...) и вступительный текст.\n"
                "2. Форматируй каждый файл в виде списка: название файла жирным (**file.py**:), со следующей строки пиши его описание, а еще ниже с новой строки указывай 'Источник: [slug]'.\n"
                "3. СТРОГО используй оригинальные названия файлов из контекста для источника (например, [code-catalog-py]). ЗАПРЕЩЕНО писать цифры вроде [1], [2]!\n"
                "4. НЕ добавляй в конце блок 'Источники', он сгенерируется автоматически.\n"
                "5. СТРОЖАЙШЕ ЗАПРЕЩЕНО переводить технические термины и классы. Оставляй их В ТОЧНОСТИ как в тексте без перевода.\n"
                "6. ВАЖНО: Описания файлов и скриптов обязательно переводи на русский язык."
            )
