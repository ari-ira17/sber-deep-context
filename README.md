...
### 3. Подготовка данных
При клонировании векторная база данных пуста. Запустите скрипт инжеста, чтобы загрузить документы из `data/dataset_with_vectors.json` и построить поисковые индексы (FTS BM25):
```bash
python scripts/ingest.py
```
**ВАЖНО:** Для работы со ссылками на вложения (изображения, исходный код) папка `meridian_hackathon_knowledge_base` должна находиться строго **в корне проекта**.
...
### 5. Прогон калибровочного бенчмарка (100 вопросов)
```bash
